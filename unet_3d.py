...
class Down2SkipAdd(torch.nn.Module):
    ...
    def __init__(self, nf, activation):
        ...
        self.front = torch.nn.Sequential(
            torch.nn.Conv3d(1, nf, 3, stride=1, padding=1),
            activation(),
            torch.nn.Conv3d(nf, nf, 3, stride=1, padding=1),
            activation(),
        )
        self.down_1 = torch.nn.Sequential(
            torch.nn.Conv3d(nf, nf * 2, 3, stride=2, padding=1),
            activation(),
        )
        self.down_2 = torch.nn.Sequential(
            torch.nn.Conv3d(nf * 2, nf * 4, 3, stride=2, padding=1),
            activation(),
        )
        self.bottom = torch.nn.Sequential(
            torch.nn.Conv3d(nf * 4, nf * 4, 3, stride=1, padding=1),
            activation(),
            torch.nn.Conv3d(nf * 4, nf * 4, 3, stride=1, padding=1),
            activation(),
            torch.nn.Conv3d(nf * 4, nf * 4, 3, stride=1, padding=1),
        ...
        self.final = torch.nn.Sequential(
            torch.nn.Conv3d(nf, nf, 3, stride=1, padding=1),
            activation(),
            torch.nn.Conv3d(nf, 1, 3, stride=1, padding=1),
            activation(),
            torch.nn.Sigmoid(),
        )
