...
def get_config():
    config = {
        ...
        'augment': [
            ...
            {'func': 'rotate_2D',
             'prob': 0.5,
             'kwargs': {'degree_range': [-180, 180]}},
            {'func': 'gaussian_noise',
             'prob': 0.2,
             'kwargs': {'scale': 0.2, 'ratio': 0.05}},
        ],
        'model': {
...
def run_training():
    ...
    train_ds = dst.RandomAugment(train_ds, config['augment'])
    ...
    valid_dl = dst.build_dataloader(
        valid_ds, config['infer_batch_size'], shuffle=False)
